...
```
The function handle one blocs at a time. In order to determine the haplotypes for the blocs created using the whole genome, we need to loop over them as follow :
...
for(chr in unique(df_blocs$chr)){
  ...
  # determine the haplotypes
  list_haplotypes = mapply(FUN     = determine_haplotypes,
                           chr     = chr,
                           start   = start,
                           end     = end,
                           bgnfile = bgnfile,
