...
lm_par_test_haplotypes <- function(X, Y, kind='all', verbose=FALSE){
  ...
  if(kind == 'block' | kind == 'all'){
    ...
    results  = data.frame(strsplit(varL[1], '_')[[1]][2], strsplit(varL[1], '_')[[1]][3], 
                          paste(sep="_",strsplit(varL[1], '_')[[1]][1], strsplit(varL[1], '_')[[1]][2], strsplit(varL[1], '_')[[1]][3]),
    ...
    results <- data.frame(phname = row.names(results), results)
    results['test'] <- 'block'
    rownames(results) <- c()
  ...
  if(kind == 'complete' | kind == 'all'){
    ...
    complete_test_results = reshape(complete_test_results, direction="long", 
            ...
            ids=row.names(complete_test_results),v.names="p_value",
            varying=list(names(complete_test_results)))
    # rename
    colnames(complete_test_results)[which(names(complete_test_results) == "id")] <- "haplotype"
    # add start stop and nb_haplo
    complete_test_results['start'] = sapply(strsplit(complete_test_results$haplotype, "_"), `[`,2)
    complete_test_results['stop'] = sapply(strsplit(complete_test_results$haplotype, "_"), `[`,3)
    complete_test_results['nb_subjects'] = nsubj
...
#~     names(results) <- c('start', 'end', 'haplotype','p_value','nb_subjects', 'nb_haplotypes')
  ...
  if(kind == 'single' | kind == 'all'){
      results = par_single_test(X, Y)
      results['nb_haplotypes'] = dim(X)[2]
      results['test'] <- 'single'
      rownames(results) <- c()
      ordered_col <- c('test', 'phname', 'start', 'end', 'haplotype','p_value','nb_subjects', 'nb_haplotypes') 
      final_results[['single']] = results[ordered_col]}
...
#~   results  = data.frame(unlist(lapply(pv_Var, function(a) lapply(a, function(b) b)))
#~                         col_orgY,
#~                         nsubj)
haplotype=unlist(lapply(varLM, function(a) lapply(a, function(b) rownames(summary(b)$coefficients)[2])))
p_value=unlist(pv_Var)
#aaa = unlist(lapply(varLM, function(a) lapply(a, function(b) (summary(b)$coefficients)[2,'Pr(>|t|)'])))
#print(p_value)
#print(aaa)
#(lapply(varLM, function(a) lapply(a, function(b) print((summary(b)$coefficients)))))
results = data.frame(
    'haplotype'=haplotype,
    'p_value'=p_value,
    'phname'=col_orgY,'nb_subjects'=nsubj,
    'start'=as.numeric(sapply(strsplit(as.character(haplotype), "_"), `[`, 2)),
    'end'=as.numeric(sapply(strsplit(as.character(haplotype), "_"), `[`, 3))
    )
